...
export function LeaderboardTable({ address }: { address: `0x${string}`}) {
  ...
  const { data: playerWins } = useReadContract({
    ...
    const playerInTop = topScoresIsArray && topScores.some((score) => score.player === address);
    const allScores = playerInTop ? topScores : topScoresIsArray ? [...topScores, { player: "You", totalWins: playerWins}] : [];


  return (
    <>
{!topScoresIsArray || topScores.length === 0 ? (
  <div className="flex w-full justify-center">No players on leaderboard yet.</div>
) : (
